...
class SplController extends Controller
{
    ...
    public function index(request $request)
    {
        ...
        return view('lembur.index',compact('template','tanggal'));
    }
    ...
    public function cek_download_excel(request $request)
    {
            ...
            $query=Lembur::query();
            $cek=$query->whereBetween('tanggal',[$mulai,$sampai]);
            if($status=='all'){

            }else{
                $cek=$query->where('approve',$status)->count();
            }
            
            $cek=$query->count();

            if($cek==0){
                echo'<div class="nitof"><b>Oops Error !</b><br><div class="isi-nitof">Tidak Tersedia</div></div>';
            }else{
                echo'@ok';
            }
    ...
    public function download_excel(request $request)
    {
            ...
            $sampai=$request->sampai;
            $status=$request->status;
            $nama='SPL'.date('ymdhis');
            return Excel::download(new SplExport($mulai,$sampai,$status), ''.$nama.'.xlsx');
    ...
    public function get_data(request $request)
    {
        ...
        $query = Vlembur::query();
        if($request->tanggal!=''){
            $data =$query->where('tanggal',$request->tanggal);
        }
        $data =$query->where('approve',0)->orderBy('id','Desc')->get();
