...
<div id="content" class="content">
    ...
    <h1 class="page-header">group <small> (tampilan daftar group)</small></h1>
    <div class="btn-group" style="margin-bottom:1%">
        <button class="btn btn-sm btn-success" onclick="location.assign(`{{url('master/group/create')}}?id=0`)"><i class="fas fa-plus"></i> Tambah </button>
        <button class="btn btn-success" onclick="$('#modal-import').modal('show')"><i class="fas fa-upload"></i> Import</button>
        
    </div>
    <div class="panel panel-inverse" data-sortable-id="form-plugins-1">
        <div class="panel-heading ui-sortable-handle">
            <h4 class="panel-title">Daftar group</h4>
        ...
        function delete_data(id){
			...
			swal({
				...
				if (willDelete) {
						$.ajax({
							type: 'GET',
							url: "{{url('master/group/delete_data')}}",
							data: "id="+id,
							success: function(msg){
								...
								var table=$('#data-table-fixed-header').DataTable();
								table.ajax.url("{{ url('master/group/get_data')}}").load();
							}
						});
