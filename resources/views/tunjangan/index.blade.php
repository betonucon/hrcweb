...
<div id="content" class="content">
    ...
    <h1 class="page-header">Tunjangan <small> (tampilan daftar tunjangan)</small></h1>
    <!-- <div class="btn-group" style="margin-bottom:1%">
        <button class="btn btn-sm btn-success" onclick="location.assign(`{{url('master/Tunjangan/create')}}?id=0`)"><i class="fas fa-plus"></i> Tambah </button>
        <button class="btn btn-success" onclick="$('#modal-import').modal('show')"><i class="fas fa-upload"></i> Import</button>
        
    </div> -->
    <div class="panel panel-inverse" data-sortable-id="form-plugins-1">
        <div class="panel-heading ui-sortable-handle">
            <h4 class="panel-title">Daftar Tunjangan</h4>
                ...
                <table id="data-table-fixed-header" class="table table-striped table-bordered table-td-valign-middle   dt-responsive display nowrap" cellspacing="0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-nowrap" width="13%">Tunjangan</th>
                            <th class="text-nowrap" width="13%">Kontribusi</th>
        ...
        var handleDataTableFixedHeader = function() {
            ...
            if ($('#data-table-fixed-header').length !== 0) {
                var table=$('#data-table-fixed-header').DataTable({
                    ...
                    responsive: true,
                    ajax:"{{ url('master/tunjangan/get_data')}}",
					columns: [
        ...
        function delete_data(id){
			...
			swal({
				...
				if (willDelete) {
						$.ajax({
							type: 'GET',
							url: "{{url('master/jabatan/delete_data')}}",
							data: "id="+id,
							success: function(msg){
								...
								var table=$('#data-table-fixed-header').DataTable();
								table.ajax.url("{{ url('master/jabatan/get_data')}}").load();
							}
						});
